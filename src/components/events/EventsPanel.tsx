              ...
              <div key={date}>
                ...
                <div className="space-y-1.5">
                  {events.map((e: EventRow) => (
                    <div key={e.id} className="group bg-amber-50 dark:bg-amber-950 border border-amber-200 dark:border-amber-800 rounded-xl px-3 py-2.5">
                      <div className="flex items-start justify-between gap-2">
                        <p className="text-sm font-medium text-amber-900 dark:text-amber-100">{e.name}</p>
                        <div className="flex items-center gap-1 shrink-0">
                          {e.recurring !== 'none' && (
                            <span className="text-[10px] text-amber-600">{RECURRING_LABELS[e.recurring] ?? e.recurring}</span>
                          )}
                          <button
